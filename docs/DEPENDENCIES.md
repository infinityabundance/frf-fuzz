...
| `blake3` | `=1.8.7` | 1.85-compatible (resolved by gemel 0.11.0's lock) | `coordinator` | Content identity (BLAKE3-256) for the object store and corpus. Chosen over SHA-256 for speed on the admission path. gated so the target-runtime build never links it. |
| `frf` | `=0.1.72` | 1.85 | `coordinator` | Epistemic authority: court questions, receipts, claims, trajectories for promoted findings. No features exist; pulls clap/serde_yaml/tar/base64/sha2/libc unconditionally — the price of real evidence semantics, kept out of the target-runtime build. Verified to compile on 1.85. |
...
* **No libAFL**: its MSRV (>= 1.93 for 0.16.x) exceeds the 1.85 contract; we
  borrow compositional ideas, not the crate.
...
* Integration-sensitive crates are pinned with `=`. Updating is a deliberate
  ...
  (`cargo +1.85.0 build`), re-run the full test suite, and update this
